...
bool RectPrism::check_point_inside(const QVec point)
{
	//lets place the point relative to the cube in 0,0,0
	QVec placed_point = placePoint(point);
	
	cout<< "Wx: "<<Wx<<"Wy: "<<Wy<<"Wz: "<<Wz<<endl;
	cout<<placed_point(0)<<" "<<placed_point(1)<<" "<<placed_point(2)<<endl;
	
  if (placed_point(0)>(Wx/2) && placed_point(0)< (-(Wx/2)) &&
			placed_point(1)>(Wy/2) && placed_point(1)< (-(Wy/2)) &&
			placed_point(2)>(Wz/2) && placed_point(2)< (-(Wz/2)))
		return true;
	else
		return false;
}
