...
using namespace RoboCompArt;
using namespace RoboCompCamera;


class artComp : public RoboComp::Application
{
private:
...
void artComp::initialize()
{
	// Config file properties read example
	// configGetString( PROPERTY_NAME_1, property1_holder, PROPERTY_1_DEFAULT_VALUE );
	// configGetInt( PROPERTY_NAME_2, property1_holder, PROPERTY_2_DEFAULT_VALUE );
}
int artComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapterArt = communicator()->createObjectAdapter("ArtComp");
		ArtI *art = new ArtI(worker);
		adapterArt->add(art, communicator()->stringToIdentity("art"));
